...
f_num = zeros(1,length(t));
f_denom = zeros(1,length(t));
f = zeros(1,length(t));
x_dmp = zeros(1,length(t));
x_dmp(:,1) = x_init;
dx_dmp = zeros(1,length(t));
ddx_dmp = zeros(1,length(t));
...
for i = 1:length(t)
    ...
    ddx_dmp(:,i) = (K.*(x_goal-x_dmp(:,i))-D.*dx_dmp(:,i)+f(:,i))/(tau);%(tau.^2);
